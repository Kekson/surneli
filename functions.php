...
/**
 * Mobile bottom sticky nav bar (Home / Blog / Shop / Wishlist / Account /
 * Cart) - same bar aromati.ge shows on mobile. Porto already ships this
 * as a built-in feature: Theme Options > General > "Mobile Bottom Sticky
 * Bar" sets the Redux option 'show-icon-menus-mobile' (+ one icon-class
 * option per item), and porto/footer/sticky-bottom.php (always called
 * from footer.php) renders it whenever that option is non-empty - it's
 * pure CSS/markup already in the parent theme, nothing to build from
 * scratch.
 *
 * This site's saved 'porto_settings' Redux option doesn't have it turned
 * on. Rather than editing that option in the DB (which Theme Options
 * re-imports/resets would wipe), force it on via the 'option_porto_settings'
 * filter - Redux's own opt_name for this theme (see
 * porto/inc/admin/theme_options/settings.php line ~226:
 * `$porto_settings = get_option('porto_settings');`) - so it survives resets
 * and needs no DB/admin access to apply.
 */
add_filter('option_porto_settings', 'surneli_enable_mobile_sticky_navbar');
function surneli_enable_mobile_sticky_navbar($settings)
{
	if (!is_array($settings)) {
		$settings = array();
	}

	// Order here is the display order in the bar (left to right).
	$settings['show-icon-menus-mobile'] = array('home', 'blog', 'shop', 'wishlist', 'account', 'cart');

	// Same icon set as aromati.ge; all but "blog" are Porto's own defaults
	// (porto/inc/admin/theme_options/menu.php, 'mobile-sticky-bar' section).
	$settings['sticky-icon-home']     = 'porto-icon-category-home';
	$settings['sticky-icon-blog']     = 'porto-icon-edit';
	$settings['sticky-icon-shop']     = 'porto-icon-bars';
	$settings['sticky-icon-wishlist'] = 'porto-icon-wishlist-2';
	$settings['sticky-icon-account']  = 'porto-icon-user-2';
	$settings['sticky-icon-cart']     = 'porto-icon-shopping-cart';
